...
import {
  ...
  Scissors, TrendingUp, Shield, Award, Clock, Users, Zap, Lock,
  AlertCircle, DollarSign, AlertTriangle, Image as ImageIcon,
  ChevronLeft, ChevronRight // ← Adicionados aqui
} from "lucide-react"
import { PROVA_SOCIAL_IMAGES } from "../lib/config"

/* ─── CONFIG ────────────────────────────────────────────────────────────────── */
const HOTMART_URL      = "https://pay.hotmart.com/SEULINK"   // ← altere aqui
const WHATSAPP_NUMBER  = "5511999999999"                      // ← altere aqui
const WHATSAPP_MSG     = encodeURIComponent("Oi, quero saber mais sobre o Método MSM")
...
const TESTIMONIALS = [
  { name: "Mariana Costa",    role: "Cabeleireira • São Paulo",      text: "Fiz outros cursos e ainda travava na execução. Com o Método MSM finalmente entendi o porquê de cada passo. Nunca mais tive medo de manchar.", stars: 5 },
  { name: "Patricia Oliveira", role: "Proprietária de Salão • Rio",   text: "Aumentei meu ticket em 40% nas mechas logo no primeiro mês. Minha cliente percebeu a diferença na segurança com que eu executei.", stars: 5 },
  { name: "Fernanda Lima",    role: "Colorista • Curitiba",           text: "O módulo de diagnóstico mudou tudo. Agora faço a anamnese certa, o teste de mecha e nunca mais pego o cabelo sem saber o que vai acontecer.", stars: 5 },
  { name: "Juliana Ramos",    role: "Cabeleireira • Belo Horizonte",  text: "Era exatamente o que eu precisava: um método claro, sem enrolação, que dá pra aplicar no salão no mesmo dia. Recomendo pra toda cabeleireira.", stars: 5 },
  { name: "Camila Santos",    role: "Freelancer • Brasília",          text: "Fiz o curso com medo de corte químico. Depois de concluir o Módulo 2 já me senti segura para atender. O suporte também é incrível.", stars: 5 },
  { name: "Rafaela Mendes",   role: "Colorista Profissional • Salvador", text: "As técnicas do Módulo 3 me salvaram. Hoje executo Free Hands e Luminous Slice com total confiança e cobro o que meu trabalho merece.", stars: 5 },
]
...
function FAQItem({ question, answer, isOpen, onClick }: { question: string; answer: string; isOpen: boolean; onClick: () => void }) {
  return (
    <div className="border-b last:border-0" style={{ borderColor: "rgba(191,161,136,0.25)" }}>
      ...
      <div className="overflow-hidden transition-all duration-500 ease-out" style={{ maxHeight: isOpen ? "28rem" : 0, paddingBottom: isOpen ? "1.5rem" : 0 }}>
...
export default function LandingPage() {
  ...
  // Multiplicamos o array original para criar a sensação natural de rolagem infinita
  const carouselImages = [...PROVA_SOCIAL_IMAGES, ...PROVA_SOCIAL_IMAGES, ...PROVA_SOCIAL_IMAGES]
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const scrollAmount = window.innerWidth > 640 ? 320 : 250 // Pulo de 1 card exato

      // Se chegar no final do array triplicado, volta pro começo
  ...
  const scrollCarousel = (direction: "left" | "right") => {
    if (carouselRef.current) {
      isPausedRef.current = true // Pausa o automático ao clicar
      ...
      // Retoma o automático 3 segundos após o clique
    ...
    <main style={{ background: "var(--bg)", overflowX: "hidden" }}>
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="absolute rounded-full" style={{
                width: `${[8,5,10,6,9,4][i]}px`, height: `${[8,5,10,6,9,4][i]}px`,
                left: `${[15,35,60,75,85,50][i]}%`, top: `${[20,60,25,70,40,80][i]}%`,
            ...
            <div className="absolute -right-32 top-1/4 w-[500px] h-[500px] rounded-full animate-blob opacity-20"
              style={{ background: "radial-gradient(circle, rgba(201,168,76,0.4), rgba(191,161,136,0.15))" }} />
            <div className="absolute -left-20 bottom-1/4 w-[350px] h-[350px] rounded-full animate-blob opacity-15"
              style={{ background: "radial-gradient(circle, rgba(169,117,98,0.3), rgba(250,235,230,0.2))", animationDelay: "-5s" }} />
          </div>
        )}

        <div className="absolute top-0 left-0 right-0 h-[3px]"
          style={{ background: "linear-gradient(90deg, transparent 0%, var(--gold-warm) 30%, var(--gold-shine) 50%, var(--gold-warm) 70%, transparent 100%)" }} />
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div>
              <div className={`transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                <div className="badge-gold mb-7">
                  ...
                  Solange Jesus Academy · Método MSM
                </div>
              </div>

              <p
                className={`transition-all duration-700 delay-75 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}
                style={{ fontFamily: "var(--font-great-vibes), cursive", fontSize: "clamp(2.2rem, 5vw, 3.8rem)", color: "var(--brown-warm)", lineHeight: 1.1, marginBottom: "0.25rem" }}
              >
              ...
              <h1
                ...
                style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(2rem, 5vw, 3.6rem)", fontWeight: 800, lineHeight: 1.1, marginBottom: "1.5rem", color: "var(--fg)" }}
              >
                Domine a técnica e{" "}
                <span className="gold-text-animated">nunca mais tenha medo de manchar</span>
              ...
              <p
                ...
                style={{ fontFamily: "var(--font-sans)", fontSize: "1.05rem", lineHeight: 1.8, color: "var(--muted)", maxWidth: "32rem", marginBottom: "0.75rem" }}
              >
                Aprenda o Método MSM para executar mechas com confiança, cobrar o que seu trabalho merece e ser reconhecida como especialista —{" "}
                <strong style={{ color: "var(--brown-deep)" }}>mesmo que você ainda se sinta insegura hoje.</strong>
              </p>

              <p
                className={`transition-all duration-700 delay-175 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
                style={{ fontFamily: "var(--font-sans)", fontSize: "0.9rem", color: "var(--muted)", marginBottom: "2.5rem" }}
              >
                Com mais de <strong style={{ color: "var(--brown-deep)" }}>19 anos de experiência</strong> na área da beleza e mechas.
              </p>
              ...
              <div className={`flex flex-col sm:flex-row gap-3 mb-10 transition-all duration-700 delay-200 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
                ...
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="btn-gold"
                  style={{ background: "transparent", color: "var(--brown-deep)", border: "1.5px solid rgba(107,79,58,0.3)", boxShadow: "none" }}>
                ...
                <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
                  {[
                    ...
                    <div key={text} className="flex items-center gap-1.5">
                      ...
                      <span style={{ fontFamily: "var(--font-sans)", fontSize: "0.8rem", color: "var(--muted)" }}>{text}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className={`hidden lg:flex justify-center items-center transition-all duration-1000 delay-400 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
              <div className="relative w-full max-w-sm">
                <div className="absolute -inset-6 rounded-3xl animate-spin-slow opacity-30"
                  style={{ background: "conic-gradient(from 0deg, var(--gold-warm), var(--gold-shine), var(--gold-light), var(--tan), var(--gold-warm))", filter: "blur(18px)" }} />
                ...
                <div className="relative rounded-2xl p-8 z-10" style={{ background: "white", boxShadow: "0 24px 64px rgba(107,79,58,0.18), 0 8px 24px rgba(107,79,58,0.1)" }}>
                  <div className="text-center mb-5">
                    ...
                    <h3 style={{ fontFamily: "var(--font-serif)", fontWeight: 700, fontSize: "1.2rem", color: "var(--fg)", marginBottom: "0.5rem" }}>
                      Mechas <span className="gold-text">Sem Medo</span>
                    </h3>
                    <p style={{ fontFamily: "var(--font-sans)", color: "var(--muted)", fontSize: "0.85rem", lineHeight: 1.6 }}>
                      O método completo para executar mechas com segurança e confiança — do diagnóstico à finalização
                    </p>
                  </div>

                  {[
                    "5 módulos + bônus de finalizações",
                  ...
                  <div className="mt-6 pt-5" style={{ borderTop: "1px solid rgba(191,161,136,0.2)" }}>
                    <a href={HOTMART_URL} target="_blank" rel="noopener noreferrer" className="btn-gold w-full" style={{ display: "flex" }}>
                      Garantir Minha Vaga
                    </a>
                  </div>
                </div>

                <div className="absolute -top-4 -right-4 w-20 h-20 rounded-full flex flex-col items-center justify-center animate-float"
                  style={{ background: "linear-gradient(135deg, var(--brown-deep), var(--gold))", boxShadow: "0 8px 24px rgba(107,79,58,0.35)" }}>
                  <p style={{ fontFamily: "var(--font-serif)", color: "white", fontWeight: 800, fontSize: "0.7rem", textAlign: "center", lineHeight: 1.1 }}>7 dias<br/>garantia</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="dark-luxury py-14 px-5 md:px-10">
        <div className="relative z-10 max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-6 divide-x divide-white/10">
            ...
            <StatCard target={98}  suffix="%"     label="de Satisfação"          />
            <StatCard target={5}   suffix=" Módulos" label="+ Bônus de Finalização" />
      ...
      <section className="py-20 md:py-32 px-5 md:px-10 gold-texture-bg">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center reveal">
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-12">
            {[
              { icon: AlertCircle, pain: "Clientes chegam pedindo mechas e você hesita em aceitar — não tem confiança de que vai ficar sem manchas na raiz" },
          ...
          <p style={{ fontFamily: "var(--font-serif)", fontSize: "1.15rem", fontWeight: 600, marginTop: "3rem", color: "var(--brown-deep)" }}>
            Isso tem solução. E o Método MSM foi criado exatamente para isso.
          </p>
        </div>
      </section>
            ...
            {BENEFITS.map(({ icon: Icon, title, text }, i) => (
              <div key={i} className="card p-7 reveal-scale" style={{ transitionDelay: `${i*80}ms` }}>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-5"
                  style={{ background: "linear-gradient(135deg, rgba(201,168,76,0.12), rgba(107,79,58,0.08))" }}>
      ...
      <section className="dark-luxury py-20 md:py-28 px-5 md:px-10">
        <div className="relative z-10 max-w-3xl mx-auto text-center reveal">
          ...
          <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(1.8rem, 4vw, 2.8rem)", fontWeight: 700, color: "white", marginBottom: "1rem" }}>
            ...
            <span className="gold-text-dark">segurança e confiança nas mechas?</span>
          </h2>
          ...
          <div className="grid lg:grid-cols-2 gap-14 xl:gap-20 items-center">
            ...
            <div className="reveal-left order-2 lg:order-1">
              <div className="relative max-w-md mx-auto">
                <div className="absolute -inset-3 rounded-3xl blur-2xl opacity-25"
                  style={{ background: "linear-gradient(135deg, var(--gold), var(--gold-light))" }} />
                <div className="relative rounded-2xl overflow-hidden"
                  style={{ background: "rgba(107,79,58,0.06)", border: "1px solid rgba(201,168,76,0.2)", aspectRatio: "4/5" }}>
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <p className="gold-text" style={{ fontFamily: "var(--font-great-vibes), cursive", fontSize: "2.2rem", marginBottom: "0.3rem" }}>
                      Solange Jesus
                    </p>
                    <p style={{ fontFamily: "var(--font-sans)", color: "var(--muted)", fontSize: "0.88rem" }}>
                ...
                <div className="absolute -bottom-5 -right-5 rounded-2xl px-5 py-3 animate-float"
                  style={{ background: "white", boxShadow: "0 10px 30px rgba(107,79,58,0.2)" }}>
      ...
      <section className="py-20 md:py-32 px-5 md:px-10" style={{ background: "rgba(250,235,230,0.4)" }}>
              ...
              <div key={i} className="card p-7 reveal-scale" style={{ transitionDelay: `${i*100}ms` }}>
                ...
                <div className="flex items-center gap-3 pt-4" style={{ borderTop: "1px solid rgba(191,161,136,0.2)" }}>
                  <div className="w-11 h-11 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0"
                    style={{ background: "linear-gradient(135deg, var(--tan), var(--brown-deep))" }}>
                    {name.charAt(0)}
                  </div>
                  <div>
        ...
        {/* ── CARROSSEL DE IMAGENS (PROVA SOCIAL) ── */}
          <div className="mt-20 max-w-6xl mx-auto reveal">
            <div className="text-center mb-10">
              <h3 style={{ fontFamily: "var(--font-serif)", fontSize: "1.5rem", fontWeight: 700, color: "var(--brown-deep)" }}>
            ...
            <div className="relative group px-0 sm:px-14">
              ...
              <button 
                ...
                className="absolute left-0 top-1/2 -translate-y-1/2 z-10 hidden sm:flex w-12 h-12 rounded-full items-center justify-center shadow-md transition-transform hover:scale-110 active:scale-95"
                style={{ background: "white", border: "1px solid rgba(191,161,136,0.3)" }}
                aria-label="Rolar para a esquerda"
              >
                <ChevronLeft size={24} style={{ color: "var(--brown-deep)" }} />
              </button>

              {/* Wrapper do Efeito de Continuidade e Controle de Pausa */}
              <div 
              ...
              >
                {/* Container do Carrossel (usando o array clonado) */}
                <div 
                  ref={carouselRef}
                  className="flex overflow-x-auto snap-x snap-mandatory gap-5 pb-6 px-4 sm:px-0 [&::-webkit-scrollbar]:hidden" 
                  style={{ scrollBehavior: "smooth", scrollbarWidth: "none", msOverflowStyle: "none" }}
                >
                  {carouselImages.map((imgSrc, index) => (
                    <div key={index} className="flex-none w-[75vw] sm:w-[300px] snap-center">
                      <div className="rounded-2xl overflow-hidden relative shadow-sm" style={{ background: "rgba(107,79,58,0.04)", aspectRatio: "4/5" }}>
                        <img 
                          src={imgSrc} 
              ...
              <button 
                ...
                className="absolute right-0 top-1/2 -translate-y-1/2 z-10 hidden sm:flex w-12 h-12 rounded-full items-center justify-center shadow-md transition-transform hover:scale-110 active:scale-95"
                style={{ background: "white", border: "1px solid rgba(191,161,136,0.3)" }}
                aria-label="Rolar para a direita"
              >
                <ChevronRight size={24} style={{ color: "var(--brown-deep)" }} />
              </button>
            </div>
            
            {/* Dica visual apenas no mobile */}
            <div className="flex justify-center mt-2 sm:hidden">
              <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.75rem", color: "var(--tan)" }}>
      ...
      <section className="py-20 md:py-32 px-5 md:px-10">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="text-center mt-10 reveal">
            ...
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="btn-gold"
              style={{ width: "auto", display: "inline-flex", padding: "0.85rem 2rem", fontSize: "0.82rem" }}>
      ...
      <section className="dark-luxury py-24 md:py-36 px-5 md:px-10">
        <div className="relative z-10 max-w-3xl mx-auto text-center reveal">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mt-10 pt-8"
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-5">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full flex items-center justify-center"
                style={{ background: "linear-gradient(135deg, var(--tan), var(--gold))" }}>
                <span style={{ fontFamily: "var(--font-great-vibes), cursive", color: "white", fontSize: "1.1rem" }}>S</span>
              </div>
              <div>
                <span style={{ fontFamily: "var(--font-serif)", color: "rgba(255,255,255,0.7)", fontSize: "1rem", display: "block" }}>Solange Jesus Academy</span>
                <span style={{ fontFamily: "var(--font-sans)", color: "rgba(255,255,255,0.35)", fontSize: "0.72rem" }}>Formando profissionais mais seguras e confiantes nas mechas</span>
              </div>
            </div>
